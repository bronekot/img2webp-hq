...
pub fn encode(
    ...
    match options.mode {
        Mode::Lossy => encode_lossy(options, decoded, color_pipeline),
        Mode::Lossless => encode_lossless(options, decoded, None),
        Mode::NearLossless(value) => encode_lossless(options, decoded, Some(value)),
    }
}
...
fn encode_lossy(
    ...
    let planes = if options.uses_simpleyuv() {
    ...
    let config = base_config(options)
        .quality(options.quality)
        .method(options.method)
        .alpha_quality(options.alpha_quality)
        .sharp_yuv(!options.uses_simpleyuv())
        .exact(options.exact);
    ...
    attach_metadata(webp, options, decoded)
}
...
fn base_config(options: &ConversionOptions) -> EncoderConfig {
    let mut config = EncoderConfig::new().thread_level(1);
    if let Some(value) = options.sns_strength {
        config = config.sns_strength(value);
    }
    if let Some(value) = options.filter_strength {
